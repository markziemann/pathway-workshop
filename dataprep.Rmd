...
## Curate count data for iDEP

Need to aggregate for duplicated gene names.
Counts for genes with the same name are summed.

```{r,curate_for_idep}

mxf$gene <- sapply(strsplit(rownames(mxf)," "),"[[",2)

mxfa <- aggregate(data=mxf, . ~ gene, FUN=sum)
dim(mxfa)
...
dir.create("data")
write.table(mxfa,"data/counts.tsv",sep="\t",row.names=FALSE)
